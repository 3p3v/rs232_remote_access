...
int FreeRTOS_read(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		/* Get data */
		rc = mbedtls_ssl_read(n->my_socket.fd, buffer, len);
		if (rc > 0)
			recvLen += rc;
		else if (rc < 0)
		{
...
int FreeRTOS_write(Network* n, unsigned char* buffer, int len, int timeout_ms)
{
	...
	do
	{
		...
		/* Send data */
		rc = mbedtls_ssl_write(n->my_socket.fd, (const unsigned char*)buffer + sentLen, len - sentLen);
		if (rc > 0)
			sentLen += rc;
		else if (rc < 0)
		{
...
{
	mbedtls_net_close(n->my_socket.fd);
}
